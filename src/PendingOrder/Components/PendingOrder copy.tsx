...
import { Row, Col, Form, Table, Alert, Button, Modal } from 'react-bootstrap';
import { PendingOrderTypes, OrderProduct } from "../Types/PendingOrderTypes";
import SuppliersSelect from "./SelectClient";
...
const PendingOrder: React.FC<PendingOrderProps> = ({ onRowClick }) => {
  ...
  const loadOrderDetails = async (orderId: number) => {
    try {
      const mockOrderData = {
        ...
        products: [
          { id: 1, productName: 'Ladrillo limpio 10 hueco', quantity: 100, price: 52, total: 5200, quantityPerTrip: 50 },
          { id: 2, productName: 'Ladrillo cálao', quantity: 555, price: 4, total: 2220, quantityPerTrip: 200 },
          { id: 3, productName: 'Cemento gris x 50kg', quantity: 20, price: 35000, total: 700000, quantityPerTrip: 10 },
        ],
  ...
  const customSave = async (onSuccess: () => void, onError: (error: any) => void) => {
    ...
    const assignment: PendingOrderTypes = {
      ...itemTemplate(),
    ...
    <div className="app-content content">
            ...
            <div style={{
              ...
              <Button
                ...
                style={{
                  ...
                  border: 'none',
                  borderRadius: '6px',
                  backgroundColor: !selectedAssignment ? '#6c757d' : '#17a2b8',
                  color: 'white'
                }}
      ...
      <Modal show={showDetailModal} onHide={handleCloseDetailModal} size="xl" centered>
        <Modal.Header closeButton style={{ backgroundColor: '#f8f9fa', borderBottom: '2px solid #dee2e6' }}>
          <Modal.Title>Detalle del Pedido Asignado</Modal.Title>
        </Modal.Header>
        <Modal.Body style={{ maxHeight: '70vh', overflowY: 'auto' }}>
          {selectedAssignment && (
            <div>
              <div style={{ backgroundColor: '#e3f2fd', padding: '15px', borderRadius: '8px', marginBottom: '20px', border: '1px solid #90caf9' }}>
                <h5 style={{ marginBottom: '15px', color: '#1976d2', fontWeight: 'bold' }}>📦 Información de la Asignación</h5>
                <Row>
                  <Col md={3}>
                    <p style={{ margin: '5px 0', fontSize: '14px' }}>
                      <strong>Transportador:</strong><br />{selectedAssignment.userName}
                    </p>
                  </Col>
                  <Col md={3}>
                    <p style={{ margin: '5px 0', fontSize: '14px' }}>
                      <strong>Bodega:</strong><br />{selectedAssignment.warehouseName}
                    </p>
                  </Col>
                  <Col md={3}>
                    <p style={{ margin: '5px 0', fontSize: '14px' }}>
                      <strong>Barrio:</strong><br />{selectedAssignment.neighborhoodName}
                    </p>
                  </Col>
                  <Col md={3}>
                    <p style={{ margin: '5px 0', fontSize: '14px' }}>
                      <strong>Fecha:</strong><br />{selectedAssignment.date}
                    </p>
                  </Col>
                </Row>
              </div>

              <div style={{ backgroundColor: '#fff3e0', padding: '15px', borderRadius: '8px', marginBottom: '20px', border: '1px solid #ffb74d' }}>
                <h5 style={{ marginBottom: '15px', color: '#f57c00', fontWeight: 'bold' }}>👤 Información del Cliente</h5>
                <Row>
                  <Col md={3}><p style={{ margin: '5px 0', fontSize: '14px' }}><strong>Nombre:</strong><br />{selectedAssignment.customerName}</p></Col>
                  <Col md={3}><p style={{ margin: '5px 0', fontSize: '14px' }}><strong>Ciudad:</strong><br />{selectedAssignment.customerCity}</p></Col>
                  <Col md={3}><p style={{ margin: '5px 0', fontSize: '14px' }}><strong>Celular:</strong><br />{selectedAssignment.customerPhone}</p></Col>
                  <Col md={3}><p style={{ margin: '5px 0', fontSize: '14px' }}><strong>Total:</strong><br />${selectedAssignment.totalPurchase.toLocaleString('es-CO')}</p></Col>
                </Row>
              </div>

              <div style={{ backgroundColor: '#f3e5f5', padding: '15px', borderRadius: '8px', marginBottom: '20px', border: '1px solid #ce93d8' }}>
                <h5 style={{ marginBottom: '15px', color: '#7b1fa2', fontWeight: 'bold' }}>📋 Productos</h5>
                <Table bordered hover>
                  <thead style={{ backgroundColor: '#7b1fa2', color: 'white' }}>
                    <tr>
                      <th>Producto</th>
                      <th style={{ textAlign: 'center' }}>Cant. Total</th>
                      <th style={{ textAlign: 'center' }}>Cant. Viaje</th>
                      <th style={{ textAlign: 'center' }}>Precio</th>
                      <th style={{ textAlign: 'center' }}>Total</th>
                    </tr>
                  </thead>
                  <tbody>
                    {selectedAssignment.products?.map((p, i) => (
                      <tr key={i}>
                        <td>{p.productName}</td>
                        <td style={{ textAlign: 'center' }}>{p.quantity}</td>
                        <td style={{ textAlign: 'center' }}>{p.quantityPerTrip}</td>
                        <td style={{ textAlign: 'center' }}>${p.price.toLocaleString('es-CO')}</td>
                        <td style={{ textAlign: 'center' }}>${p.total.toLocaleString('es-CO')}</td>
                      </tr>
                    ))}
                  </tbody>
                </Table>
